...
class UserLogic extends BaseLogic
{
    ...
    public static function info(int $userId)
    {
        $user = User::where(['id' => $userId])
            ->field('id,sn,sex,account,password,nickname,real_name,avatar,mobile,update_time,user_money,role,id_card,bank_name,bank_account,profile,city_address')
            ->findOrEmpty();
        $user['has_password'] = !empty($user['password']);
     ...
     * @date 2023/9/21 16:53
     */
    public static function SaveInfo(array $params)
    {
        try {
            return User::update(
                $params
