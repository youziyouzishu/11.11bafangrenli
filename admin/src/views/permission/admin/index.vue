                    ...
                    <el-table-column label="名称" width="150" show-overflow-tooltip>
                        <template #default="{ row }">
                            <view class="flex items-center">
                                {{ row.enterpriseVerification?.org_name ?? '未认证' }}
                            </view>
                        </template>
                    </el-table-column>
                    <el-table-column label="角色" prop="role_name" min-width="100" show-tooltip-when-overflow />
                    <el-table-column label="余额" min-width="150" v-perms="['auth.admin/edit']">
                        <template #default="{ row }">
                            <view class="flex items-center">
                                ¥{{ row.user_money }}
                                <el-button v-if="row.enterpriseVerification?.org_name" v-perms="['auth.admin/edit']"
                                    type="primary" link @click="handleAdjust(row.user_money, row.id)">
                                    调整
                                </el-button>
                            </view>
                        </template>
...
import { adminEdit, adminLists, adminDelete, adjustMoney } from '@/api/perms/admin'
