...
use app\common\http\tim\{TencentUserSign, IMAccountImporter};
...
class LoginLogic extends BaseLogic
{
    ...
    public static function loginOrRegister($params)
    {
        try {
            // 账号/手机号 密码登录
            $where = ['account|mobile' => $params['account']];
            ...
            IMAccountImporter::AddOne("cl_{$user['sn']}", $user["nickname"]);
            $user->im_user_sign =  TencentUserSign::Computer("cl_{$user["sn"]}");
            ...
            return [
                ...
                'token' => $userInfo['token'],
                'role' => $userInfo['role'],
                'uniid' => $userInfo['uniid'] ?? NULL,
                'sex' => $userInfo['sex'] ?? 0,
                'flow_status' => $user->flow_status ?? 0,
                'realname_status' =>  PersonalVerification::where(['user_id' => $userInfo['user_id']])->value('realname_status') ?? 0,
            ];
        } catch (\Exception $e) {
            self::setError($e->getMessage());
            return false;
