...
class LoginLogic extends BaseLogic
{
    ...
    public function login($params)
    {
        ...
        $redirect_url = null;
        if (in_array(1,$admin['role_id']) && empty($admin['agreement_no'])){
            //人力公司没有开通资金监管协议
            Pay::config(config('payment'));
            $param = array();
            // 设置个人签约产品码
            $param['personal_product_code'] = 'FUND_SAFT_SIGN_WITHHOLDING_P';
            $param['sign_scene'] = 'INDUSTRY|SATF_ACC';
            $param['access_params'] = ['channel'=>'QRCODE'];
            $param['product_code'] = 'FUND_SAFT_SIGN_WITHHOLDING';
            $param['external_agreement_no'] = $admin->id;
            $param['third_party_type'] = 'PARTNER';
            $param['_method'] = 'GET';
