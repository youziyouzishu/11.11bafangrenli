...
class AuthController extends BaseAdminController
{
    ...
    public function callback()
    {
        try {
                    ...
                    switch ($flowInfo['data']['authType']) {
                        case "ORG":
                            ...
                            if ($flowInfo["data"]["realNameStatus"] == 1 && $flowInfo["data"]["authorizedStatus"] == 1) {
                                ...
                                $find = EnterpriseVerification::where('auth_flow_id', $result['authFlowId'])->find();
                                if ($find) {
                                    $find->psn_mobile = $authinfo['person']['psnAccount']['accountMobile'] ?? '';
                                    $find->psn_id = $authinfo['person']['psnId'] ?? '';
                                    $find->realname_status = 4;
                                    $find->org_name = $authinfo['organization']['orgName'] ?? '';
                                    $find->org_id = $authinfo['organization']['orgId'] ?? '';
                                    $find->org_id_card_num = $authinfo['organization']['orgInfo']['orgIDCardNum'] ?? '';
                                    $find->org_id_card_type = $authinfo['organization']['orgInfo']['orgIDCardType'] ?? '';
                                    $find->legal_rep_name = $authinfo['organization']['orgInfo']['legalRepName'] ?? '';
                                    $find->legal_rep_id_card_num = $authinfo['organization']['orgInfo']['legalRepIDCardNum'] ?? '';
                                    $find->legal_rep_id_card_type = $authinfo['organization']['orgInfo']['legalRepIDCardType'] ?? '';
                                    $find->save();
                                    $find->admin->name = $authinfo['organization']['orgName'] ?? '';
                                    $find->admin->save();
                                }
                            }
                            break;
            ...
            EsignLogHelper::writeLog($e->getMessage(), 'callback');
            EsignLogHelper::writeLog($e->getLine(), 'callback');
        }
    }
}
